...
#SBATCH --output=/work/smtitann/output/space/binary_%x_bwt_runs_%j.dat # The location, where all output is collected. %j will be replaced with the job id.
cd $1 # Go to the directory, where the executable is stored.
cargo run --release binary $2 bwt_runs 1 $3
