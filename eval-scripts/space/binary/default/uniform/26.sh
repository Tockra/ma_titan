#!/bin/bash -l
#SBATCH --time=00:04:00      # The maximum wall time of this job is two hours.
...
#SBATCH --output=/work/smtitann/output/space/binary_uniform_26_%j.dat # The location, where all output is collected. %j will be replaced with the job id.
cd $1 # Go to the directory, where the executable is stored.
cargo run --release binary $2 uniform 26 $3