...
#SBATCH --output=/work/smtitann/output/space/binary_uniform_6_%j.dat # The location, where all output is collected. %j will be replaced with the job id.
cd $1 # Go to the directory, where the executable is stored.
cargo run --release binary $2 uniform 6 $3