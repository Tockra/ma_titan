...
#SBATCH --output=/work/smtitann/output/space/stree_%x_normal_9_%j.dat # The location, where all output is collected. %j will be replaced with the job id.
cd $1 # Go to the directory, where the executable is stored.
cargo run --release stree $2 normal 9 $3