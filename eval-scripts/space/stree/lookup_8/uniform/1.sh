...
#SBATCH --output=/work/smtitann/output/space/stree_%x_uniform_1_%j.dat # The location, where all output is collected. %j will be replaced with the job id.
cd $1 # Go to the directory, where the executable is stored.
cargo run --release stree $2 uniform 1 $3
