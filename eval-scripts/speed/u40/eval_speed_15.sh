#!/bin/bash -l
#SBATCH --time=06:00:00      # The maximum wall time of this job is two hours.
#SBATCH --partition=med    # Hence, we can use the short queue.
...
#SBATCH --output=/work/smtitann/output/speed/pred/binary_%x_bwt_runs_%j.dat # The location, where all output is collected. %j will be replaced with the job id.
cd $1 # Go to the directory, where the executable is stored.
cargo run --release binary pred $2 bwt_runs $3
