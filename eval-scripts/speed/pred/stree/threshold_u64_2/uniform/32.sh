#!/bin/bash -l
#SBATCH --time=32:00:00      # The maximum wall time of this job is two hours.
...
#SBATCH --output=/work/smtitann/output/speed/pred/stree_%x_uniform_32_%j.dat # The location, where all output is collected. %j will be replaced with the job id.
cd $1 # Go to the directory, where the executable is stored.
cargo run --release stree pred $2 uniform 32 $3