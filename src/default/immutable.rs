...
/// Die L2-Ebene ist eine Zwischenebene, die mittels eines u16-Integers und einer Threshold-Hashtabelle auf eine
/// L3-Ebene zeigt.
pub type L2Ebene<T> = LevelPointer<L3Ebene<T>, T>;

/// Die L3-Ebene ist eine Zwischenebene, die mittels eines u16-Integers und einer Threshold-Hashtabelle auf
...
/// Statische Predecessor-Datenstruktur. Sie verwendet eine Threshold-Hashtabelle und ein Array auf der Element-Listen-Ebene.
...
impl<T, E> Level<T, E> {
    ...
    /// eine Threshold-Hashtabelle generiert wird.
    ///
    /// # Arguments
    ///
    /// * `j` - Falls eine andere Ebene auf diese mittels Hashfunktion zeigt, muss der verwendete key gespeichert werden.
    /// * `keys` - Eine Liste mit allen Schlüsseln, die mittels Threshold-Hashtabelle auf die nächste Ebene zeigen.
    #[inline]
    pub fn new(
        lx_top: TopArray<E, u16>,
        objects: Box<[T]>,
    ...
    /// Mit Hilfe dieser Funktion kann die Threshold-Hashtabelle verwendet werden.
    /// Es muss beachtet werden, dass sichergestellt werden muss, dass der verwendete Key auch existiert!
    ///
    /// # Arguments
    ///
    /// * `key` - u16-Wert mit dessen Hilfe das zu `key` gehörende Objekt aus dem Array `objects` bestimmt werden kann.
    #[inline]
