...
impl<T: Int> STree<T> {
    ...
    pub fn new(elements: Box<[T]>) -> Self {
        ...
        unsafe {println!("STree ({}) angelegt. Anzahl der Level: {}, Anzahl der echten Hashfunktionen: {}",st.len(),LEVEL_COUNT.load(Ordering::SeqCst),HASH_FUNCTION_COUNT.load(Ordering::SeqCst));}
        st
    }
