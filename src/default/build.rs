            ...
            if self.root_table[i].is_null() {
                ...
                match self.root_table[i].get() {
                    PointerEnum::First(l) => {
                        ...
                        let val = Box::new(Level::new(GLOBAL,second_level.lx_top.take().unwrap(),
                                ...
                                for &j in &l2_level.keys {
                                    ...
                                    if (*l).get(j).is_null() {
                                        ...
                                        *pointered_data =  match l3_level.get() {
                                            PointerEnum::First(l2) => {
                                                let l3_level = l2;
                                                let mut level = Level::new(GLOBAL,l3_level.lx_top.take().unwrap(), vec![0; l3_level.keys.len()].into_boxed_slice(), l3_level.keys.clone().into_boxed_slice(),l3_level.minimum,l3_level.maximum);
