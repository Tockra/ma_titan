        ...
        for i in 0..tmp.capacity() {
            if self.root_table[i].is_null() {
                ...
                match self.root_table[i].get() {
                    PointerEnum::First(l) => {
                        let second_level = l;
                        let val = Box::new(Level::new(GLOBAL, std::mem::replace(&mut second_level.lx_top,Box::new([])),vec![LevelPointer::from_null(); second_level.keys.len()].into_boxed_slice(), second_level.keys.clone().into_boxed_slice(),second_level.minimum, second_level.maximum));
