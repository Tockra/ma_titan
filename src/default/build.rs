...
impl<T: Int> STreeBuilder<T> {
    ...
    /// Baut ein Array `root_table` für den STree-Struct. Dabei werden zuerst die `Level`-Structs korrekt mittels neuer Threshold-Hashtabelleen
    /// angelegt und miteinander verbunden. Nachdem die Struktur mit normalen Hashfunktionen gebaut wurde können nun Threshold-Hashtabelleen berechnet
    /// werden!
    pub fn build(&mut self) -> Box<[L2Ebene<T>]> {
        let mut tmp: Vec<L2Ebene<T>> = Vec::with_capacity(T::root_array_size());
        // Die L2Level-Elemente werden angelegt. Hierbei wird direkt in der new()-Funktion die Threshold-Hashtabelle berechnet
...
pub struct BuilderLevel<T, E> {
    /// Klassische HashMap zum aufbauen der Threshold-Hashtabelle
