        ...
        match data_structure.root_table[i].get() {
            PointerEnum::First(l) => {
                ...
                let saved_val = match second_level.get() {
                    PointerEnum::First(l) => unsafe { **l.get(k) },
                    PointerEnum::Second(e) => *e,
                };
                assert_eq!(saved_val, val);
            }
    ...
    for val in check {
        ...
        if data_structure.root_table[i].minimum() != data_structure.root_table[i].maximum() {
            ...
            if second_level.minimum() != second_level.maximum() {
                ...
                assert_eq!(unsafe {**saved_val}, val);
            } else {
                assert_eq!(*second_level.minimum(), val);
            }
        } else {
            assert_eq!(
                *data_structure.root_table[i].minimum(),
                val
            );
        }
    }
...
fn test_u40_locate_or_succ_eqc_bruteforce_test() {
    ...
    assert_eq!(
        data_structure.locate_or_succ(u40::new(0b11111111111111111111_1111111111_1111111111_u64)),
        None
    );
    ...
    for (i, &elem) in data.iter().enumerate() {
        if i > 0 {
            ...
            assert_eq!(
                *data_structure.locate_or_succ(elem - u40::new(1)).unwrap(),
                elem
            );
        }
    }
...
fn test_u48_locate_or_succ_eqc_bruteforce_test() {
    ...
    assert_eq!(
        data_structure.locate_or_succ(u48::new(
    ...
    for (i, &elem) in data.iter().enumerate() {
        if i > 0 {
            ...
            assert_eq!(
                *data_structure.locate_or_succ(elem - u48::new(1)).unwrap(),
                elem
            );
        }
    }
...
fn test_u40_locate_or_pred_bruteforce() {
    ...
    assert_eq!(
        u40::new(1065983),
        *data_structure.locate_or_pred(u40::new(1065983)).unwrap()
    );
...
fn test_u48_locate_or_pred_bruteforce() {
    ...
    assert_eq!(
        u48::new(1065983),
        *data_structure.locate_or_pred(u48::new(1065983)).unwrap()
    );
    for (index, _) in data_v1.iter().enumerate().rev() {
        if index > 0 {
            for i in (data_v1[index - 1]..data_v1[index]).rev() {
...
fn test_u40_locate_or_pred_eqc_bruteforce_test() {
    ...
    for (i, &elem) in data.iter().enumerate().rev() {
        if i < data.len() - 1 {
            ...
            assert_eq!(
                *data_structure.locate_or_pred(elem + u40::new(1)).unwrap(),
                elem
            );
        }
    }
...
fn test_u48_locate_or_pred_eqc_bruteforce_test() {
    ...
    for (i, &elem) in data.iter().enumerate().rev() {
        if i < data.len() - 1 {
            ...
            assert_eq!(
                *data_structure.locate_or_pred(elem + u48::new(1)).unwrap(),
                elem
            );
        }
    }
